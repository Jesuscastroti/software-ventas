...
$(document).ready(function() {
	...
	$("#addClientModalBtn").unbind('click').bind('click', function() {
		...
		$(".form-group").removeClass('has-error').removeClass('has-success');
			...
			if(clientId == "") {
					$("#clientId").after('<p class="text-danger">Este campo es obligatorio</p>');
					$('#clientId').closest('.form-group').addClass('has-error');
			}	else {
					// remov error text field
					$("#clientId").find('.text-danger').remove();
			...
			if(comenCliente == "") {
				...
				if($clientId && NameClient && AddresClient && EmailClient  && TelClient && CatCliente && comenCliente) {
